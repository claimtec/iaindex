-- ============================================================================
-- IAIndex v2.0 - Fresh Project Migration
-- ============================================================================
-- Description: Complete schema for fresh Supabase installation
-- This consolidates all tables needed for the AI visibility SaaS platform
--
-- IMPORTANT: Run this FIRST on a brand new Supabase project
-- After running this, run the additional migrations in order:
--   1. 001_schema_pivot_migration.sql (AI visibility features)
--   2. 002_email_automation.sql (Email automation)
--   3. 003_analytics_tables.sql (Analytics)
-- ============================================================================
...
-- This is auto-populated via trigger when users sign up through Supabase Auth
...
-- Function to auto-create profile when user signs up via Supabase Auth
...
CREATE INDEX idx_publishers_verified ON publishers(domain_verified);
CREATE INDEX idx_publishers_policy_config ON publishers USING GIN (policy_config);
...
CREATE INDEX idx_receipts_purpose ON receipts(purpose_type);
CREATE INDEX idx_receipts_commercial ON receipts(commercial);
...
CREATE TABLE IF NOT EXISTS attestations (
    ...
    attestation_type VARCHAR(50) NOT NULL CHECK (attestation_type IN ('c2pa', 'merkle', 'blockchain')),
...
-- Merkle indexes
CREATE INDEX idx_merkle_roots_hash ON merkle_roots(root_hash);
CREATE INDEX idx_merkle_roots_timestamp ON merkle_roots(timestamp DESC);
CREATE INDEX idx_merkle_nodes_root ON merkle_nodes(root_id);
CREATE INDEX idx_merkle_nodes_receipt ON merkle_nodes(receipt_id);
CREATE INDEX idx_merkle_timestamps_root ON merkle_timestamps(merkle_root_id);
CREATE INDEX idx_merkle_timestamps_blockchain ON merkle_timestamps(blockchain);
CREATE INDEX idx_merkle_timestamps_status ON merkle_timestamps(status);
CREATE INDEX idx_merkle_timestamps_created ON merkle_timestamps(created_at DESC);
...
CREATE INDEX idx_bot_reputation_client ON bot_reputation(client_id);
CREATE INDEX idx_bot_reputation_status ON bot_reputation(status);
CREATE INDEX idx_bot_reputation_score ON bot_reputation(reputation_score);
CREATE INDEX idx_bot_reputation_updated ON bot_reputation(updated_at DESC);
CREATE INDEX idx_violations_client ON violation_records(client_id);
CREATE INDEX idx_violations_type ON violation_records(violation_type);
CREATE INDEX idx_violations_timestamp ON violation_records(timestamp DESC);
CREATE INDEX idx_violations_severity ON violation_records(severity);
CREATE INDEX idx_fraud_logs_client ON fraud_detection_logs(client_id);
CREATE INDEX idx_fraud_logs_receipt ON fraud_detection_logs(receipt_id);
CREATE INDEX idx_fraud_logs_type ON fraud_detection_logs(fraud_type);
CREATE INDEX idx_fraud_logs_detected ON fraud_detection_logs(detected_at DESC);
CREATE INDEX idx_fraud_logs_severity ON fraud_detection_logs(severity);
CREATE INDEX idx_fraud_logs_resolved ON fraud_detection_logs(resolved);

-- Bot reputation triggers
...
CREATE TABLE IF NOT EXISTS user_preferences (
    ...
    receipt_notifications BOOLEAN DEFAULT FALSE,
...
CREATE POLICY attestations_service ON attestations FOR ALL USING (true);
...
CREATE POLICY user_preferences_service ON user_preferences FOR ALL USING (true);
...
GRANT ALL ON publishers TO authenticated;
GRANT ALL ON receipts TO authenticated;
GRANT ALL ON attestations TO authenticated;
GRANT ALL ON user_preferences TO authenticated;

GRANT SELECT ON bot_reputation TO authenticated;
GRANT SELECT ON violation_records TO authenticated;
GRANT SELECT ON fraud_detection_logs TO authenticated;

GRANT ALL ON profiles TO anon;
GRANT ALL ON publishers TO anon;
GRANT ALL ON receipts TO anon;
GRANT ALL ON attestations TO anon;
...
SELECT 'Migration 000 completed successfully!' as message;
